...
export default function JobDetailClient({ id }: { id: string }) {
  ...
  return (
    <div className="space-y-5 pb-12">
      ...
      <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm space-y-3">
        <div className="flex items-start justify-between">
          <div>
            ...
            <a
              href="#venue-card"
              className="flex items-center gap-1.5 text-xs text-slate-600 mt-1 hover:text-indigo-600 transition group cursor-pointer"
            >
              <MapPin className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
              <span className="font-semibold text-slate-800 group-hover:text-indigo-700 underline decoration-indigo-200">
      ...
      {/* 9. 웨딩홀 인텔리전스 2단 카드 (상단: 팩트시트 / 하단: 디어메모리 크루 노트) */}
      {space && (
        <section id="venue-card" className="space-y-2 scroll-mt-20">
